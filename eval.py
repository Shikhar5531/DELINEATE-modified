...
test_generator, test_samples, filenames = dataLoader(opt.data_path+'/val/', 1,  img_shape, train_mode=False)
...
with sess.as_default():
    ...
    for it in range(0, test_samples):
        ...
        base_name = os.path.basename(filenames[it])
        vis.save_seg(pred_map, name=base_name)

        print ('[iter %f]: meanIU=%f' % (it, score))
